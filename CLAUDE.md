...
0. **`saveMyGame()` (the 💾 Save my game button; hub account row + Parent page).** Folder-first, download-last: tries save server → File System Access folder (`connectBackupFolder` picked once, then `getLastDirHandle`+`ensureRWPermission`, writes via `writeHistoryToFolder`) → `saveToDownload()` (anchor download with `download="saved_status/kls-save-<stamp>.json"`; **note browsers sanitize the `/` → a flat file `saved_status_kls-save-….json` in Downloads, NOT a folder** — verified in Chromium). Must be called from a user gesture (Save click) for the picker/permission prompt. Returns `{mode:'server'|'folder'|'download'|'cancelled'}`. The File System Access picker is blocked on `file://` (verified: opaque origin → SecurityError) → graceful download fallback. **Only the save server (launcher) reliably produces an actual `saved_status/` folder.** Autosave-to-file is impossible on `file://` (sandbox).

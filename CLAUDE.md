...
0. **`saveMyGame()` (the 💾 Save my game button; hub account row + Parent page).** Folder-first, download-last: tries save server → File System Access folder (`connectBackupFolder` picked once, then `getLastDirHandle`+`ensureRWPermission`, writes via `writeHistoryToFolder`) → `saveToDownload()` (anchor download of `saved_status/kls-save-<stamp>.json`). Must be called from a user gesture (Save click) for the picker/permission prompt. Returns `{mode:'server'|'folder'|'download'|'cancelled'}`. The picker may be blocked on `file://` in some browsers → graceful download fallback; autosave-to-file is impossible on `file://` (sandbox).

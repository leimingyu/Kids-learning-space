...
0. **`saveToDownload()` (works on plain double-click / `file://`).** The **💾 Save my game** button (hub account row + Parent page) downloads `saved_status/kls-save-<stamp>.json` via an anchor download — Chrome/Edge create a `saved_status` subfolder in Downloads, others flatten. This is the only file a `file://` page can produce; a page cannot write next-to-the-game or autosave-to-file (browser sandbox).
